...
class TestTemplateMCPServer:
    ...
    def test_server_initialization_failure(self):
        ...
        with patch('template_mcp_server.src.mcp.FastMCP', side_effect=Exception("FastMCP error")):
    ...
    def test_register_mcp_tools_called(self):
        ...
        with patch('template_mcp_server.src.mcp.FastMCP') as mock_fastmcp:
            ...
            with patch.object(TemplateMCPServer, '_register_mcp_tools') as mock_register:
                TemplateMCPServer()
    ...
    def test_tool_registration_order(self):
        ...
        with patch('template_mcp_server.src.mcp.FastMCP') as mock_fastmcp:
            ...
            expected_tools = ['multiply_numbers', 'read_redhat_logo_content', 'get_code_review_prompt']
    ...
    def test_server_logging_on_success(self):
        ...
        with patch('template_mcp_server.src.mcp.FastMCP'):
            with patch('template_mcp_server.src.mcp.logger') as mock_logger:
                TemplateMCPServer()
                mock_logger.info.assert_called_with("Template MCP Server initialized successfully")

    def test_server_logging_on_failure(self):
        """Test that failure is logged when server initialization fails."""
        with patch('template_mcp_server.src.mcp.FastMCP', side_effect=Exception("Test error")):
            with patch('template_mcp_server.src.mcp.logger') as mock_logger:
                with pytest.raises(Exception):
                    TemplateMCPServer()
                
                mock_logger.error.assert_called_with("Failed to initialize Template MCP Server: Test error")
