...
from rfe_mcp_server.src.oauth.handler import SCOPE, OAuth2Handler


class TestOAuth2Handler:
    """Test class for OAuth2Handler."""

    @patch("rfe_mcp_server.src.oauth.handler.settings")
    def test_create_oauth_session_without_state(self, mock_settings):
        ...
        with patch("rfe_mcp_server.src.oauth.handler.OAuth2Session") as mock_oauth:
            mock_session = Mock()
    ...
    @patch("rfe_mcp_server.src.oauth.handler.OAuth2Handler.create_oauth_session")
    ...
    @patch("rfe_mcp_server.src.oauth.handler.settings")
    @patch("rfe_mcp_server.src.oauth.handler.httpx.post")
    ...
    @patch("rfe_mcp_server.src.oauth.handler.OAuth2Handler.introspect_token")
    ...
    @patch("rfe_mcp_server.src.oauth.handler.OAuth2Handler.verify_access_token")
...
class TestOAuth2HandlerIntegration:
    ...
    @patch("rfe_mcp_server.src.oauth.handler.settings")
    @patch("rfe_mcp_server.src.oauth.handler.OAuth2Session")
    @patch("rfe_mcp_server.src.oauth.handler.httpx.post")
