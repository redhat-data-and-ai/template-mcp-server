.PHONY: help install clean test lint format coverage pre-commit local kill container deploy undeploy deps
...
kill: ## Force-stop the MCP server (SIGKILL; bypasses SIGTERM because the uvicorn dev server often ignores graceful shutdown)
